/* Bundled action entrypoint. Generated by npm run build:action. */
"use strict";
...
function locateAfter(path, source, needle, anchor) {
  const lines = source.split(/\r?\n/u);
  const offset = lines.slice(0, Math.max(0, anchor.line - 1)).reduce((total, line) => total + line.length + 1, 0);
  return locateFromOffset(path, source, needle, offset);
}
function locateFromOffset(path, source, needle, offset) {
...
function permissionLevel(permissions, name) {
...
function findTaintSources(path, source, value, anchor) {
    ...
    for (const match of text.matchAll(pattern)) {
      ...
      if (expression !== void 0) {
        results.push({
          expression,
          label,
          location: anchor === void 0 ? locate(path, source, expression) : locateAfter(path, source, expression, anchor)
        });
      }
    }
  }
...
function stringifyValue(value) {
  ...
  if (value !== null && typeof value === "object") {
    return Object.entries(value).map(([key, nested]) => `${key}: ${stringifyValue(nested)}`).join("\n");
  }
...
var UNTRUSTED_TRIGGERS = /* @__PURE__ */ new Set([
  "discussion",
  "discussion_comment",
  "issue_comment",
  "issues",
...
function evaluateWorkflow(workflow) {
  ...
  const untrustedTrigger = workflow.triggers.some(
    (trigger) => UNTRUSTED_TRIGGERS.has(trigger)
  );
...
function evaluatePullRequestTarget(workflow, job) {
  if (!workflow.triggers.includes("pull_request_target")) {
    return [];
  }
  return job.steps.filter((step) => /actions\/checkout/iu.test(step.uses ?? "")).filter(
    (step) => /github\.event\.pull_request\.(?:head\.sha|head\.ref|head\.repo)/u.test(
...
function makeFinding(workflow, job, step, ruleId, message, nodes) {
  ...
  return {
    ...
    fingerprint: (0, import_node_crypto.createHash)("sha256").update(identity).digest("hex").slice(0, 24)
  };
}
function rule(id) {
...
function trace(kind, label, location) {
  return { kind, label, location };
}
function suppressAndDeduplicate(source, findings) {
  const suppressions = [
    ...source.matchAll(/agent-flow-audit:\s*ignore\s+(AFA\d{3})/gu)
  ].map((match) => {
    const id = match[1];
    if (id === void 0 || match.index === void 0) {
      return void 0;
    }
    return {
      id,
    ...
    (suppression) => suppression !== void 0
  );
  const seen = /* @__PURE__ */ new Set();
  return findings.filter((finding) => {
    const locallySuppressed = suppressions.some(
      (suppression) => suppression.id === finding.ruleId && suppression.line <= finding.location.line && finding.location.line - suppression.line <= 2
    );
    ...
    runs: [
      {
        tool: {
          driver: {
            ...
            informationUri: "https://github.com/loriadatj-cyber/agent-flow-audit",
            rules: RULES.map((rule2) => ({
              id: rule2.id,
              name: rule2.title,
              shortDescription: { text: rule2.title },
              fullDescription: { text: rule2.description },
              help: {
                text: rule2.remediation,
                markdown: `**Remediation:** ${rule2.remediation}`
              },
              defaultConfiguration: {
                level: sarifLevel(rule2.severity)
              },
              properties: {
                precision: rule2.id === "AFA004" ? "medium" : "high",
...
function formatText(result, options = {}) {
  ...
  lines.push(
    `Scanned ${result.files.length} workflow${result.files.length === 1 ? "" : "s"}: ${result.findings.length} finding${result.findings.length === 1 ? "" : "s"} (${counts.critical} critical, ${counts.high} high, ${counts.medium} medium), ${result.errors.length} parse error${result.errors.length === 1 ? "" : "s"}.`
  );
  return lines.join("\n");
}
...
function paint(text, severity, enabled) {
  ...
  const code = severity === "critical" || severity === "high" ? 31 : severity === "medium" ? 33 : 36;
...
function parseWorkflow(path, source) {
  return /\.md(?:own)?$/iu.test(path) ? parseAgenticMarkdown(path, source) : parseActionsYaml(path, source);
}
function parseActionsYaml(path, source) {
...
function parseAgenticMarkdown(path, source) {
  ...
  const engine = typeof metadata.engine === "string" ? metadata.engine : "agentic-workflow";
...
// src/scanner.ts
var SUPPORTED_EXTENSIONS = /\.(?:ya?ml|md|markdown)$/iu;
async function scan(options = {}) {
  ...
  for (const absolutePath of files) {
    ...
    try {
      const source = await (0, import_promises.readFile)(absolutePath, "utf8");
...
async function discover(path) {
  let metadata;
  try {
    metadata = await (0, import_promises.stat)(path);
  } catch {
    return [];
  }
  if (metadata.isFile()) {
  ...
  const nested = await Promise.all(
    entries.filter((entry) => entry.name !== ".git" && entry.name !== "node_modules").map((entry) => discover((0, import_node_path.resolve)(path, entry.name)))
  );
  return nested.flat();
}
...
async function run() {
  const paths = input("paths", ".github/workflows").split(/\r?\n|,/u).map((path) => path.trim()).filter(Boolean);
...
`);
  await (0, import_promises2.writeFile)(sarifFile, formatSarif(result), "utf8");
  for (const finding of result.findings) {
...
function input(name, fallback) {
  ...
  return process.env[githubKey]?.trim() || process.env[shellFriendlyKey]?.trim() || fallback;
}
